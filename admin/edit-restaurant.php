...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Handle photo processing (Up to 10 photos across kept existing, new links, and uploaded image files)
    $kept_photos = [];
    if (isset($_POST['existing_photos']) && is_array($_POST['existing_photos'])) {
        foreach ($_POST['existing_photos'] as $url) {
            $url = trim($url);
            if (!empty($url)) $kept_photos[] = $url;
        }
    }
    ...
    $photo_urls = array_values(array_unique(array_merge($kept_photos, $new_photo_urls, $uploaded_photo_urls)));
    ...
    try {
        ...
        if ($pdo) {
            ...
            $stmtUpd = $pdo->prepare("
                UPDATE restoran 
                SET nama_restoran = ?, deskripsi = ?, id_kecamatan = ?, alamat = ?, jam_operasional_weekday = ?, maps = ?, no_wa = ?
                WHERE id_restoran = ?
            ");
            $stmtUpd->execute([
                sanitize($_POST['name']),
            ...
            if (!empty($photo_urls)) {
                $pdo->prepare("UPDATE restoran SET foto_utama = ? WHERE id_restoran = ?")->execute([$photo_urls[0], $id]);
                $pdo->prepare("DELETE FROM foto_restoran WHERE id_restoran = ?")->execute([$id]);
                foreach ($photo_urls as $purl) {
                    $pdo->prepare("INSERT INTO foto_restoran (id_restoran, url_foto) VALUES (?, ?)")->execute([$id, $purl]);
                }
            }
            ...
            foreach ($parsed_menus as $m) {
                ...
                $stmtMenu->execute([$id, $m['name'], $m['price']]);
            }
        }
    } catch (Throwable $e) {}
...
if (!$restaurant) redirect('restoran.php');
...
<form method="POST" action="" enctype="multipart/form-data">
    ...
    <!-- Photo Management Card (Up to 10 photos total: existing kept + direct uploads + URL links) -->
    <div class="adm-card" style="margin-bottom:20px;">
        <div class="adm-card-header" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
            ...
            <div style="display:flex; align-items:center; gap:8px;">
                <span id="photo-count-badge" style="background:rgba(45, 106, 63, 0.12); color:var(--adm-green); padding:5px 14px; border-radius:30px; font-weight:700; font-size:0.82rem; font-family:'Poppins',sans-serif; display:flex; align-items:center; gap:6px;">
                    <i class="fa-solid fa-camera"></i> <span id="photo-total-count">0</span> / 10 Foto Tersimpan
                </span>
            </div>
        </div>
        <div class="adm-card-body" style="display:flex; flex-direction:column; gap:24px;">
            <?php $photos = !empty($restaurant['photos']) ? $restaurant['photos'] : (!empty($restaurant['foto_utama']) ? [$restaurant['foto_utama']] : []); ?>

            <!-- Section 1: Galeri Foto Saat Ini / Sebelumnya -->
            <div id="existing-photos-section">
                <div style="font-weight:700; font-size:0.9rem; color:var(--adm-text); margin-bottom:8px; display:flex; align-items:center; gap:6px;">
                    <i class="fa-solid fa-folder-open" style="color:var(--adm-green);"></i> Foto Saat Ini (<span id="existing-count-label"><?= count($photos) ?></span> foto diinput sebelumnya)
                </div>
                <p style="font-size:0.8rem; color:var(--adm-text-muted); margin-bottom:14px;">Berikut adalah foto-foto yang sudah ada di database. Klik tombol hapus (🗑️) jika ingin menghapus foto tertentu sebelum menyimpan.</p>
                
                <?php if (empty($photos) || (count($photos) === 1 && empty(trim($photos[0])))): ?>
                    <div id="no-existing-photos-msg" style="padding:20px; text-align:center; background:var(--adm-bg); border-radius:8px; border:1px dashed var(--adm-card-border); color:var(--adm-text-muted); font-size:0.85rem;">
                        Belum ada foto tersimpan untuk restoran ini.
                    </div>
                    <div id="existing-photos-grid" style="display:grid; grid-template-columns:repeat(auto-fill, minmax(140px, 1fr)); gap:14px;"></div>
                <?php else: ?>
                    <div id="no-existing-photos-msg" style="display:none; padding:20px; text-align:center; background:var(--adm-bg); border-radius:8px; border:1px dashed var(--adm-card-border); color:var(--adm-text-muted); font-size:0.85rem;">
                        Belum ada foto tersimpan untuk restoran ini.
                    </div>
                    <div id="existing-photos-grid" style="display:grid; grid-template-columns:repeat(auto-fill, minmax(140px, 1fr)); gap:14px;">
                        <?php foreach($photos as $idx => $val): if(empty(trim($val))) continue; ?>
                        <div class="existing-photo-item" style="position:relative; border-radius:10px; overflow:hidden; border:2px solid var(--adm-card-border); background:var(--adm-bg); display:flex; flex-direction:column; aspect-ratio:1;">
                            <input type="hidden" name="existing_photos[]" value="<?= htmlspecialchars($val) ?>" class="existing-photo-input">
                            <img src="<?= htmlspecialchars($val) ?>" style="width:100%; height:100%; object-fit:cover; flex:1;" referrerpolicy="no-referrer">
                            <div style="position:absolute; bottom:0; inset-x:0; background:rgba(0,0,0,0.72); backdrop-filter:blur(3px); color:#fff; font-size:0.72rem; font-weight:700; padding:4px 8px; display:flex; justify-content:space-between; align-items:center;">
                                <span>#<?= $idx+1 ?> <?= $idx===0?'(Utama)':'' ?></span>
                                <button type="button" onclick="removeExistingPhoto(this)" title="Hapus foto ini" style="background:#ef4444; border:none; color:#fff; width:22px; height:22px; border-radius:4px; cursor:pointer; display:flex; align-items:center; justify-content:center;">
            ...
            <!-- Section 2: Unggah Foto Baru Langsung dari Komputer / HP -->
            <div>
            ...
            <!-- Section 3: Input Foto Berupa Link URL -->
            <div>
    ...
    function removeExistingPhoto(btnElem) {
        const item = btnElem.closest('.existing-photo-item');
        if (item) {
            item.remove();
            const grid = document.getElementById('existing-photos-grid');
            if (grid && grid.querySelectorAll('.existing-photo-item').length === 0) {
                const msg = document.getElementById('no-existing-photos-msg');
                if (msg) msg.style.display = 'block';
            }
            // Update labels #1, #2...
            document.querySelectorAll('.existing-photo-item').forEach((elem, i) => {
                const span = elem.querySelector('span');
                if (span) span.innerText = '#' + (i + 1) + (i === 0 ? ' (Utama)' : '');
            });
            updatePhotoCounter();
        }
    }
    ...
    function handleFileSelect(inputElem) {
        ...
        const currentCount = getExistingCount() + getUrlCount();
    ...
    function getExistingCount() {
        return document.querySelectorAll('.existing-photo-item').length;
    }
    ...
    function getTotalPhotosCount() {
        return getExistingCount() + getFileCount() + getUrlCount();
    }
    ...
    function updatePhotoCounter() {
        ...
        const total = existingCount + fileCount + urlCount;
        ...
        if (existingLabel) existingLabel.innerText = existingCount;
        ...
        if (badge) {
            if (total >= 10) {
                ...
                badge.innerHTML = `<i class="fa-solid fa-camera"></i> <span>${total}</span> / 10 Foto Tersimpan`;
            }
        }
